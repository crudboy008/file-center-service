...
```mermaid
flowchart TD
    C[FileController / FileStatusController<br/>HTTP DTO、校验与错误映射] --> S[FileService / FileStatusService<br/>业务编排与事务边界]
...
| 所有者 | 拥有的数据 | 禁止的耦合 |
|---|---|---|
| Java 文件中心 | 逻辑文件/内容版本身份、元数据、对象引用、SHA-256、生命周期、可用状态、状态历史与并发版本 | Python 不直接读写 Java 表 |
