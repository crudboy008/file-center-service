...
| 部署单元 | 形态 | 职责 | 本期状态 |
|---|---|---|---|
| Java 卡业务应用 | `card-service-app` 生成的一个 Spring Boot 可执行 JAR；当前装配 `file-center-service` | 统一 Java 运行宿主；文件模块提供文件 HTTP API、双状态、历史、业务规则和 PostgreSQL/MinIO 编排 | 必需 |
...
```mermaid
flowchart LR
    CLIENT[前端/测试客户端] -->|HTTP| APP[card-service-app<br/>建议本地端口 18080]
    subgraph JVM[同一个 Java JVM]
        APP --> FILE[file-center-service]
    end
    FILE -->|JDBC| PG[(PostgreSQL 15.19<br/>建议本地映射 15432)]
    FILE -->|S3 API| MINIO[(MinIO<br/>建议 API 19000<br/>Console 19101)]
```
...
- Java 卡业务应用唯一启动宿主 `card-service-app`，其中装配 `file-center-service`；
- PostgreSQL 15.19；
- MinIO 固定本地版本。
...
未来启动对象：Gateway、Java 卡业务应用（文件模块和后续 Java 业务模块仍共用一个 JVM）、Python RAG/Agent、RocketMQ、MinIO、双方数据库，以及按届时需求启用的 Nacos/Sentinel。

该剖面必须另立实施与验收文档。在它被真实执行前，状态固定为 `NOT_RUN`，不能由 `contract` 或 `file-integration` 的结果推导为通过。

## 5. 配置分级

| 配置类型 | 示例 | 责任位置 | 存放/注入方式 |
|---|---|---|---|
| 整体构建配置 | Java 版本、BOM、插件、模块清单 | `E:\project\java-card-service\pom.xml` | 父 POM；子模块继承，不在文件模块重复锁版本 |
| 整体本地编排 | PostgreSQL/MinIO 镜像、端口、网络 | 整体根 `docker-compose.yml` | 由整体工程统一维护；不放入 `file-center-service` |
| 跨模块应用配置 | HTTP 端口、Profile、DataSource、Jackson、日志、请求 ID、全局异常响应、OpenAPI 聚合 | `card-service-app` | `card-service-app/src/main/resources/application*.yml` 和启动宿主配置类 |
| 文件专属配置 | 上传开关、允许类型、分页上限、Bucket 逻辑名、补偿参数 | 配置类型与校验在 `file-center-service`，运行值在 `card-service-app` | 启动宿主配置文件或环境变量注入；后续可迁移到 Nacos Config |
| 环境地址 | PostgreSQL URL、MinIO Endpoint | `card-service-app` 运行环境 | 环境变量或部署平台配置 |
| 凭据与密钥 | 数据库密码、MinIO Secret Key、JWT 私钥 | 进程运行环境 | 环境变量或部署平台密钥能力；禁止进入 Git、Nacos 明文样本和文档 |
| 契约常量 | schema version、Topic、Tag、file_ref 前缀、MessageGroup 规则 | 版本化契约 | 契约文档/Schema；未来运行配置只能选择已支持值 |
| 业务事实 | 文件对象键、SHA-256、生命周期、可用状态、状态历史、并发版本 | 文件模块拥有的数据表 | PostgreSQL；不得由动态配置改写历史记录 |

配置优先级、变量名和样例值由 LLD 定义。`file-center-service` 不得创建自己的整体 `application.yml`、启动类、Wrapper 或 Compose；文件模块只声明文件专属 `@ConfigurationProperties` 类型和校验，由 `card-service-app` 提供运行值。任何样例都不得包含真实 AccessKey、密码、Token 或私钥。

当前磁盘事实是整体根 POM 和 `card-service-app` 尚不存在，Git 根仅位于 `file-center-service`。实施可以设计上述整体文件，但不得自行移动/初始化 Git；整体根与启动宿主的版本控制归属需由用户另行确定。
...
- 本地模拟环境允许同一 Java 应用的业务模块复用 PostgreSQL 进程和应用级 DataSource；文件模块拥有自己的文件表/Schema 写入口，其他模块不得绕过文件 Service 修改；
