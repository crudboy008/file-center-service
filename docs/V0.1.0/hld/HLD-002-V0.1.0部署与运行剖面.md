...
| 部署单元 | 形态 | 职责 | 本期状态 |
|---|---|---|---|
| Java 文件中心 | 一个 Spring Boot 可执行 JAR | 文件 HTTP API、业务规则、PostgreSQL/MinIO 编排 | 必需 |
| PostgreSQL | 独立进程/容器 | 文件元数据、状态、并发版本、迁移历史 | 必需 |
...
目的：在不启动外部中间件时验证 Java DTO、JSON Schema 和固定样本。
...
4. 真实执行上传、详情、分页、下载、元数据更新、内容替换和删除。
5. 故障注入覆盖数据库提交失败、对象删除失败、重复删除和并发替换。
6. 检查对象字节、SHA-256、MinIO 元数据、数据库对象引用和状态一致。
...
| 契约常量 | schema version、Topic、Tag、file_ref 前缀 | 版本化契约文档/Schema；未来运行配置只能选择已支持值 |
| 业务事实 | 文件对象键、SHA-256、状态、并发版本 | PostgreSQL；不得由动态配置改写历史记录 |
...
最低日志关联字段：`request_id`、`file_id`、`reference_id`、操作类型和最终状态。日志不得记录文件正文、认证凭据、数据库密码、MinIO Secret Key 或完整 Token。
