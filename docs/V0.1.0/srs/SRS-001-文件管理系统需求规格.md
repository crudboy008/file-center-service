...
需求冲突时，优先级为用户最新要求 `SRC-006`～`SRC-008`，再到 `SRC-001`、`SRC-002`，均高于外部历史基线。外部基线中要求本版真实接入 RocketMQ/Python RAG 的内容被 `SRC-001`、`SRC-008` 覆盖；删除 Gateway 的历史结论被 `SRC-002` 覆盖，但 Gateway 仍属于后续版本，不进入 V0.1.0 实现与验收。

**依据：** `[SRC-001][SRC-002][SRC-006][SRC-007][SRC-008][SRC-004:159-186][SRC-005:131-144]`。

## 1. 目的

V0.1.0 交付一个可独立运行和验收的 Java 文件管理后端。系统使用 PostgreSQL 保存文件元数据、系统生命周期、人工可用状态和状态历史，使用 MinIO 保存文件字节，提供文件上传、查询、下载、元数据修改、完整文件替换、启用/停用、受控失败恢复和删除能力。
...
**依据：** `[SRC-001][SRC-006][SRC-007][SRC-008][SRC-003:39-46][SRC-003:159-181]`。
...
5. 用与证据等级相匹配的措辞报告结果：Java 文件闭环可以独立验收，RAG 只允许报告“契约已设计/静态校验”，不得报告“已联通”。
...
```mermaid
flowchart LR
    C[HTTP API 调用方] --> J[Java 文件管理后端]
    J --> P[(PostgreSQL 文件元数据)]
    J --> M[(MinIO 私有 Bucket 文件字节)]
```

V0.1.0 只有 Java 文件管理后端、PostgreSQL 和 MinIO 参与功能执行。API 调用方可以是 Swagger UI、自动化测试或其他 HTTP 客户端；本版不交付前端页面。

**依据：** `[SRC-001][SRC-003:39-60]`。
...
| 编号 | 级别 | 需求 |
|---|---|---|
| `FR-CFG-001` | MUST | 数据库地址、账号、密码必须通过外部配置注入；真实密码不得写入仓库或文档。 |
| `FR-CFG-002` | MUST | MinIO Endpoint、Access Key、Secret Key、Bucket 必须通过外部配置注入；Secret Key 不得进入日志或错误响应。 |
| `FR-CFG-003` | MUST | 单文件大小上限、单请求大小上限和允许媒体类型策略必须可配置，并在启动或部署验收对象中记录实际值。 |
| `FR-CFG-004` | MUST | 补偿任务的执行周期、单次处理上限、重试次数或退避规则必须可配置。 |
...
| 编号 | 级别 | 需求 |
|---|---|---|
| `NFR-MAIN-001` | MUST | Controller 只能调用 Service，不得直接调用 Mapper 或 MinIO Client；文件 Service 不得依赖 MinIO SDK 类型作为公共接口，对象存储通过项目自定义的 `ObjectStorageService` 与 `storage/minio` 实现隔离。 |
| `NFR-MAIN-002` | MUST | 组件版本及其状态（冻结、候选、待实测）由 HLD 的单一版本基线维护，构建和部署文件负责实际强制版本。 |
...
**依据：** `[SRC-003:63-65][SRC-001][SRC-004:73-157]`。
