...
1. 数据库不保存文件 BLOB，只保存稳定文件 ID、对象引用、大小、媒体类型、SHA-256、ETag、状态和并发版本等元数据。
...
7. 同一文件的替换、删除通过 `row_version` 条件更新，更新行数为 0 表示并发冲突，不能静默覆盖。
...
| `REPLACING` | 已预留新候选对象，内容替换尚未收口 | 是，只读取旧的已提交当前对象 | 否 | 否 | 否 |
...
sequenceDiagram
    ...
    C->>A: 上传文件
    A->>A: 校验请求并生成 file_id/reference_id/operation_id
...
1. 上传前生成稳定 `file_id`、新的 `reference_id` 和本次 `operation_id`。
...
5. PutObject 和完整性事实确认后，第二个 PostgreSQL 本地事务以 `file_id + operation_id + status=UPLOADING + row_version` 做 CAS，填充 size、SHA-256、ETag 和当前对象引用，转为 `ACTIVE`，并在同一事务完成本次候选清理任务。
...
sequenceDiagram
    ...
    M-->>A: 成功
    A->>P: 事务二：按 operation_id/row_version CAS<br/>切换引用并 REPLACING -> ACTIVE
    alt 切换并提交成功
        P-->>A: 新引用生效，补偿目标改为旧对象
        A->>M: 删除旧对象或交由补偿任务
        A-->>C: 替换成功
...
1. 新内容永远写新 Object Key；替换请求也必须拒绝实际零字节内容并返回 `FILE_EMPTY`。
2. 事务一以 `file_id + expected row_version + status=ACTIVE` 做 CAS，转为 `REPLACING`，保存旧的当前对象事实、新的候选 Object Key、`operation_id` 和候选清理任务。CAS 失败时不调用 MinIO。
3. PutObject 成功后，事务二只允许同一 `operation_id` 和预期 `row_version` 把候选对象原子切换为当前对象，并将状态恢复为 `ACTIVE`。
...
7. `REPLACING` 超时由对账任务按 `operation_id` 判定数据库当前引用和候选对象状态，再选择完成切换、恢复旧 `ACTIVE` 或转 `FAILED`。不能只看 MinIO 中“有对象”就推断切换成功。
8. `REPLACING` 期间，普通下载继续读取替换开始前已提交的当前对象，以维持文件可用性；候选对象在事务二成功切换前绝不能被下载、详情响应或 RAG 引用读取。元数据修改、再次替换和删除仍被拒绝。
...
- 按 `file_id` 查询 `ACTIVE` 或 `REPLACING` 元数据，再由基础设施层只使用已提交的当前对象引用读取；`REPLACING` 的候选引用不进入下载 DTO，也不能由客户端指定；
...
- `ACTIVE` 记录的对象必须存在；
- 超时 `REPLACING` 必须核对旧引用、候选引用、`operation_id` 和补偿任务，收敛到旧/新 `ACTIVE` 或 `FAILED`；
- `DELETING` 记录应推进删除重试；
...
- 替换处于 `REPLACING` 时，下载仍返回旧的已提交当前对象且字节/SHA-256 不变，候选对象不可见；
