...
V0.1.0 的实现范围是 Java 文件管理：PostgreSQL 保存文件元数据、生命周期和补偿事实，MinIO 保存文件字节。内容包括上传、分页查询、详情、流式下载、元数据修改、使用新 Object Key 的内容替换和幂等删除。

Java 与 Python RAG 之间只冻结对接合同和固定样例。本版不创建或运行 RocketMQ Producer/Consumer、Java RAG 状态表、Python RAG/Agent、Spring Cloud Gateway、Nacos 或 Sentinel，也不执行 Java 与 Python 的真实联通。
...
- [SRS-001 文件管理系统需求规格](srs/SRS-001-文件管理系统需求规格.md)：功能范围、非功能要求、错误语义、RAG 静态合同要求和验收条件。
...
- [LLD-001 文件管理模块详细设计](lld/LLD-001-文件管理模块详细设计.md)：Controller/Service/Mapper 包结构、数据模型、HTTP API、状态机和错误映射。
- [LLD-002 数据库与 MinIO 一致性设计](lld/LLD-002-数据库与MinIO一致性设计.md)：上传、替换、删除、恢复、补偿和对账时序。
- [LLD-003 RAG 预留对接合同](lld/LLD-003-RAG预留对接合同.md)：入库请求、接纳结果、最终结果、文件引用、幂等和兼容策略。
...
| RAG 消息字段与分支 | `lld/contracts/rag/*.schema.json` | 双方模型、序列化测试、MQ 初始化和跨语言合同测试 |
...
- `REPLACING` 期间只允许查询详情和下载替换前已提交的旧当前对象；候选对象不得进入详情、下载或 RAG 引用。
...
- Python 必须在后续版本实现 acceptance 严格模型、同业务事务 Outbox 和固定向量测试，Java 才能进入真实接入。
...
当前文档与 Schema 最多构成设计证据和静态合同证据。它们不能证明 Java 文件功能已实现，不能证明 PostgreSQL 或 MinIO 已启动，不能证明 RocketMQ 已收发消息，也不能证明 Python RAG、Gateway 或 Java/Python 端到端链路已运行。
