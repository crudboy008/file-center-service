...
| 核对 ID | 核对内容 | 最低证据 |
|---|---|---|
| `VC-DB-001` | 检查 Flyway/真实 Schema 不含文件 BLOB；验证唯一约束、状态约束和对象引用约束生效 | E2 |
...
| `VC-ARCH-001` | 检查 Controller 只调用 Service；Service 使用 Mapper 和 `ObjectStorageService`；MyBatis-Plus 类型只在 Mapper/数据访问代码；MinIO SDK 类型只在 `storage/minio`；Job 只调用 `FileReconcileService`；不存在 Controller 直连 Mapper/MinIO Client | E1 |
...
| `FR-FILE-006` | MUST | ACTIVE/REPLACING 流式下载 | LLD-001 §5.4；HTTP §6 | `AC-FILE-006`、`008` | 直接 | E2 | `DESIGN_MAPPED / NOT_RUN` |
...
| 需求 ID | 级别 | 摘要 | 设计/契约 | 验收映射 | 覆盖 | 最低证据 | 当前状态 |
|---|---|---|---|---|---|---|---|
| `FR-RAG-001` | MUST | Java → RAG 版本化请求 Schema | LLD-003 §4；intake Schema | `AC-RAG-001`、`004`、`005` | 直接 | E1 | `DESIGN_MAPPED / NOT_RUN` |
| `FR-RAG-002` | MUST | 文档/操作身份、file_ref、SHA 和访问字段 | LLD-003 §4、§8；intake Schema | `AC-RAG-001`、`004`、`005` | 直接 | E1 | `DESIGN_MAPPED / NOT_RUN` |
| `FR-RAG-003` | MUST | RAG → Java 接纳结果三分支 | LLD-003 §5；acceptance Schema | `AC-RAG-002`、`004`、`005` | 直接 | E1 | `DESIGN_MAPPED / NOT_RUN` |
| `FR-RAG-004` | MUST | 接纳分支字段存在/省略严格互斥 | LLD-003 §5；acceptance Schema | `AC-RAG-002`、`004`、`005` | 直接 | E1 | `DESIGN_MAPPED / NOT_RUN` |
| `FR-RAG-005` | MUST | RAG → Java 最终结果 Schema | LLD-003 §6；result Schema | `AC-RAG-003`、`004`、`005` | 直接 | E1 | `DESIGN_MAPPED / NOT_RUN` |
| `FR-RAG-006` | MUST | READY/FAILED 必填、省略和禁止字段 | LLD-003 §6；result Schema | `AC-RAG-003`、`004`、`005` | 直接 | E1 | `DESIGN_MAPPED / NOT_RUN` |
| `FR-RAG-007` | MUST | file_ref、MinIO 元数据和 SHA 规则 | LLD-003 §8；intake Schema | `AC-RAG-001`、`004`、`005` | 直接 | E1 | `DESIGN_MAPPED / NOT_RUN` |
| `FR-RAG-008` | MUST | 机器可校验正反样例 | RAG `examples/`、validator、manifest | `AC-RAG-001`～`004` | 直接 | E1 | `DESIGN_MAPPED / NOT_RUN` |
...
| `NFR-MAIN-003` | MUST | RAG Schema 显式版本和未知版本拒绝 | LLD-003 §4～§6、§12；RAG Schema | `AC-RAG-001`～`004` | 直接 | E1 | `DESIGN_MAPPED / NOT_RUN` |
| `NFR-MAIN-004` | MUST | Java 验收不依赖后续组件 | HLD-002 §4；`VC-SCOPE-001` | `AC-FILE-001`～`014` + `VC-SCOPE-001` | 直接+补充 | E2/E3 | `DESIGN_MAPPED / NOT_RUN` |

## 6. 当前汇总

- Java 文件功能：`NOT_IMPLEMENTED / NOT_RUN`。
- HTTP OpenAPI：`NOT_IMPLEMENTED / NOT_RUN`，且第 4 节列出的部分字段仍待冻结。
- RAG 静态合同：文档和机器可读文件存在；按当前 SRS 验收状态仍为 `NOT_RUN`。
