...
| 并发 | 修改、替换和删除使用 `expectedVersion`；过期版本返回 `409 FILE_VERSION_CONFLICT` |
| 时间 | JSON 时间使用带时区的 RFC 3339 字符串；设计样例使用 UTC `Z` |
| 分页 | 页码从 1 开始；排序固定为 `created_at DESC, file_id DESC`；默认只返回 `ACTIVE` |
...
| `FILE-LIST` | `GET /api/v1/files` | `page`、`pageSize`、`name`、`status`、`mediaType`、`createdFrom`、`createdTo` | `200 OK`；稳定分页，默认只返回 `ACTIVE` | `FR-FILE-004`、`012` |
| `FILE-DETAIL` | `GET /api/v1/files/{fileId}` | 路径参数 `fileId` | `200 OK`；`REPLACING` 返回旧的已提交内容事实，不暴露候选对象 | `FR-FILE-005`、`012` |
| `FILE-DOWNLOAD` | `GET /api/v1/files/{fileId}/content` | 路径参数 `fileId` | `200 OK` 字节流；`ACTIVE/REPLACING` 可下载当前已提交对象 | `FR-FILE-006`、`010`、`012` |
...
| `FILE-CONTENT-REPLACE` | `PUT /api/v1/files/{fileId}/content` | multipart：`file`、`expectedVersion` | 使用新内容引用原子切换；精确成功响应体/状态码待 OpenAPI 冻结 | `FR-FILE-008`、`011` |
...
| `status` | enum | 外部可见生命周期状态 |
| `version` | integer | 对应内部 `row_version` 的并发版本 |
| `createdAt` | RFC 3339 date-time | 创建时间 |

详情响应是否增加 `remark/updatedAt`，分页响应的容器字段名称，以及 PATCH/PUT 的精确成功响应体，当前设计尚未冻结。这些项目必须在实现 OpenAPI 前收口，并同步本文件和需求追踪矩阵。
...
- 替换生成新的 `reference_id/object_key`；`REPLACING` 期间查询和下载仍使用旧的已提交对象。
...
| 409 | `FILE_STATE_CONFLICT` | 当前状态不允许操作 |
...
需求与契约的验收映射以 [RTM-V0.1.0](../governance/RTM-V0.1.0.md) 为唯一矩阵。当前所有 HTTP 运行项仍为 `NOT_IMPLEMENTED / NOT_RUN`。
